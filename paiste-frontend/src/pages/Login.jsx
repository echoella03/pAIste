...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      params.append('username', form.email); params.append('password', form.password)
      const res = await API.post('/auth/login', params)
      login({ name: res.data.name, role: res.data.role }, res.data.access_token)
      navigate(res.data.role === 'admin' ? '/admin/dashboard' : '/identify')
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'white' }}>
      ...
      <div className="flex-1 flex items-center justify-center px-4">
        <div className="rounded-2m shadow-lg p-10 w-full max-w-sm" style={{ backgroundColor: '#FFFFF9' }}>
          ...
          {error && <div className="bg-red-50 text-red-600 text-sm px-4 py-2 rounded-lg mb-4 text-center font-manjari">{error}</div>}
          ...
          <p className="text-center text-sm font-manjari text-gray-500 mt-4">
            ...
            <Link to="/register" className="font-bold hover:underline" style={{ color: '#0D3A24' }}>Register</Link>
          </p>
        </div>
      </div>
    </div>
